...
$sqlRange = "SELECT 
             ...
             FROM products p
             WHERE (p.deleted = 0 OR p.deleted IS NULL) AND p.status = 'active'";
$range = $pdo->query($sqlRange)->fetch();

$rangeMin = floor($range['min_val'] / 1000) * 1000; // redondeo hacia abajo
$rangeMax = ceil($range['max_val'] / 1000) * 1000;  // redondeo hacia arriba

// valores seleccionados para el slider (si no hay filtro, usa globales)
$selMin = (isset($_GET['min_price']) && is_numeric($_GET['min_price']))
    ? (float)$_GET['min_price']
    : $rangeMin;

$selMax = (isset($_GET['max_price']) && is_numeric($_GET['max_price']))
...
// ---------- Total y offset ----------
$sqlCount = "SELECT COUNT(*) FROM products p WHERE $where";
$stmtCount = $pdo->prepare($sqlCount);

if (isset($_GET['min_price'])) {
    $stmtCount->bindValue(':min_price', $selMin, PDO::PARAM_INT);
}
if (isset($_GET['max_price'])) {
    $stmtCount->bindValue(':max_price', $selMax, PDO::PARAM_INT);
}
...
$sql = "
  SELECT 
    p.id, p.name, p.slug, p.price, p.discount_price, p.view_before_cart,
...
if (isset($_GET['min_price'])) {
    $stmt->bindValue(':min_price', $selMin, PDO::PARAM_INT);
}
if (isset($_GET['max_price'])) {
    $stmt->bindValue(':max_price', $selMax, PDO::PARAM_INT);
}
...
function urlWith(array $merge = []) {
  ...
  // Eliminar parámetros de paginación y precio si se cambia sort o limit
...
// ---------- Etiquetas en español para el sort ----------
switch ($sort) {
  case 'price_asc':  
    $sort_label = "Precio: Menor a Mayor"; 
    break;
  case 'price_desc': 
    $sort_label = "Precio: Mayor a Menor"; 
    break;
  case 'name_asc':   
    $sort_label = "Nombre: A - Z"; 
    break;
  case 'position':
  default:           
    $sort_label = "Predeterminado"; 
    break;
}

// ---------- Variables para el slider en JS ----------
       ...
       <?php include __DIR__ . "/widgets/side-menu.php";?>
        ...
        <div class="sidebar-module-container">
          <div class="sidebar-filter">         
            ...
            <!-- ============================================== PRICE SILDER============================================== -->
          <?php include __DIR__ . "/widgets/filter_price.php";?>
            <!-- /.sidebar-widget --> 
            <!-- ============================================== PRICE SILDER : END ============================================== -->       
